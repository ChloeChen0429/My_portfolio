...
def pick_emotion_chord(*args, **kwargs) -> str:
    return "C"
...
def choose_block_layers(pool: List[Dict], chord: str, energy: float, mem: RecentMem):
    # 1) Kick – always present
    kick = softmax_pick(pool, chord, energy, {"Kick"}, 1, TEMP, mem)

    # 2) Sub Bass – stable, no pitch shift
    bass = softmax_pick(pool, chord, energy, {"Bass"}, 1, TEMP, mem)

    # 3) Main Stab – the hypnotic repeating part of Subzero
    stab = softmax_pick(pool, chord, energy,
                        {"Pad","Synth","Chord","Keys","Pluck"}, 1, TEMP, mem)

    # 4) Minimal hat/noise
    hat = softmax_pick(pool, chord, energy, {"Top", "Perc", "Ride"}, 1, TEMP, mem)
    ...
    try:
        ...
        while not stop_flag.is_set():
            ...
            try:
                ...
                last_emotion = lbl  # 记录这次情绪，用于下一段音乐 + OSC
