...
try:
    ...
    HAVE_OSC = True
    print(f"📡 OSC client ready → {TD_IP}:{TD_PORT}")
except Exception as e:
    print("⚠️ python-osc not available, OSC output disabled:", e)
    osc_client = None
    HAVE_OSC = False


def emo_to_switch_index(emo: str) -> int:
    """
    Map normalised emotion label → TD Switch index.
    sad0 disgust1 neutral2 angry3 surprise4 happy5 fear6
    """
    e = (emo or "").lower()
    if e == "sad":        return 0
    if e == "disgust":    return 1
    if e == "neutral":    return 2
    if e == "angry":      return 3
    if e in ("surprise", "surprised"): return 4
    if e == "happy":      return 5
    if e == "fear":       return 6
    # 默认走 neutral
    return 2
...
def send_osc_emotion(emo: str):
    if not HAVE_OSC or osc_client is None:
        return
    idx = emo_to_switch_index(emo)
    try:
        osc_client.send_message("/vj_switch", idx)
        print(f"📡 OSC → /vj_switch {idx} (emotion={emo})")
    except Exception as e:
        print("⚠️ OSC send failed:", e)
...
CANDIDATE_AUDIO_ROOTS = [
    ENV_AUDIO_DIR,
    "/Users/chenyuxin/Downloads/My_portfolio-main/An_emotionDriven_audiovisual_system/Berlin underground",
    "/Users/chenyuxin/Downloads/My_portfolio-main/An_emotionDriven_audiovisual_system/Berlin underground",
]
...
MODEL_PATH = "/Users/chenyuxin/Downloads/My_portfolio-main/An_emotionDriven_audiovisual_system/club_trainaug_best_20251005-203557.pt"
...
LOOPS_CSV = os.path.join(DATA_DIR, "loop_meta_v3_tonal_only.csv")      # optional
AUTO_CSV  = os.path.join(DATA_DIR, "loop_meta_v2_autogen.csv")

# ========= Track structure CSV (techno_learn_final_fixed.csv) =========
STRUCT_CSV = os.path.join(DATA_DIR, "techno_learn_final_fixed.csv")

def load_structure_model(path: str) -> List[Dict]:
    rows = []
    if not os.path.isfile(path):
        print(f"⚠️ STRUCT_CSV not found: {path} (will fall back to default levels)")
        return rows
    try:
        with open(path, "r", encoding="utf-8-sig", newline="") as f:
            ...
            for r in reader:
                try:
                    ...
                    energy = 0.5
                try:
                    levels = int(float(r.get("levels", "5")))
                except Exception:
                    levels = 5
                section = str(r.get("section", "mid")).strip()
                rows.append({
                    "energy": energy,
                    "levels": levels,
                    "section": section,
                })
        print(f"🎚 Loaded structure model from {path} (rows: {len(rows)})")
    except Exception as e:
        print("⚠️ Failed to load STRUCT_CSV, using default structure:", e)
        rows = []
    return rows

STRUCT_MODEL = load_structure_model(STRUCT_CSV)

def predict_levels_from_energy(energy: float, default_levels: int = 4) -> int:
    ...
    sorted_rows = sorted(
        STRUCT_MODEL,
        key=lambda r: abs(float(r["energy"]) - float(energy))
    )
    subset = sorted_rows[:K]
    if not subset:
        return default_levels
    avg = sum(r["levels"] for r in subset) / float(len(subset))
    lvl = int(round(avg))
    if lvl < 2: lvl = 2
    if lvl > 4: lvl = 4
    return lvl
...
PREBUILD_SEC=2.0            
DETECT_WIN_SEC=0.8
# Safety margin so the voted label arrives before the next block is built.
DECISION_MARGIN_SEC=0.5
SR=44100
CALM_CHORD="Am"
DEFAULT_ENERGY=0.60
TEMP=0.7
...
# Base chord templates per emotion; will be filtered by actual library roots.
EMO_TO_CHORDS_BASE = {
    "neutral":   ["Am","C","Em","Dm"],
    "happy":     ["C","G","D","A"],
    "surprised": ["G","A","D","E"],
...
TONAL={"Bass","Pad","Synth","Lead","Arp","Chord","Keys","Pluck","Melody"}
CORE ={"Kick","Clap","Drum"}
...
def pitch_shift(seg: AudioSegment, semis: int) -> AudioSegment:
    if seg is None:
        return seg
    # semitone_shift() already chooses the shortest interval (-6..+6).
    # Keep that full interval: clamping it to +/-1 prevents a loop from
    # reaching most requested chord roots.
    semis = int(clamp(semis, -6, 6))
    if semis == 0:
        return seg
    ...
    return seg._spawn(
        seg.raw_data,
        overrides={'frame_rate': int(seg.frame_rate * rate)}
...
def read_loops_csv(path:str) -> List[Dict]:
    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for r in reader:
            ...
            key = key.replace("b","#")
            # The final tonal CSV may not contain an energy column.  In that
            # case estimate a simple, reproducible proxy from the audio file
            # so category-dependent energy targets can affect selection.
            energy_value = r.get("energy")
            try:
                en=float(energy_value) if str(energy_value or "").strip() else estimate_energy_quick(fixed)
            except Exception:
                en=estimate_energy_quick(fixed)
...
# ========= emotion-conditioned music profiles =========
# These profiles describe musical arrangement choices, not psychological
# meanings.  A recognised facial-expression label selects a different set of
# loop groups for the next block.  The mapping is deliberately kept at the
# level of visible labels and musical parameters.
LOOP_GROUPS = {
    "kick":       {"Kick"},
    "bass":       {"Bass"},
    "tonal":      {"Pad", "Synth", "Chord", "Keys", "Pluck"},
    "top":        {"Top", "Ride"},
    "percussion": {"Perc", "Drum"},
    "clap":       {"Clap"},
}

MUSIC_PROFILES = {
    "neutral":   {"target_energy": 0.60, "groups": ["kick", "bass", "tonal", "top"]},
    "happy":     {"target_energy": 0.75, "groups": ["kick", "bass", "tonal", "top", "clap"]},
    "surprised": {"target_energy": 0.70, "groups": ["kick", "bass", "tonal", "top", "percussion", "clap"]},
    "sad":       {"target_energy": 0.65, "groups": ["kick", "bass", "tonal"]},
    "angry":     {"target_energy": 0.70, "groups": ["kick", "bass", "tonal", "percussion", "clap"]},
    "fear":      {"target_energy": 0.55, "groups": ["kick", "tonal", "top"]},
    "disgust":   {"target_energy": 0.50, "groups": ["kick", "bass", "tonal", "percussion"]},
}


def get_music_profile(label: str) -> Dict:
    return MUSIC_PROFILES.get(normalize_label(label), MUSIC_PROFILES["neutral"])
...
def pick_emotion_chord(label: str, current_chord: str,
                       emotion_pools: Dict[str, List[str]],
                       temp: float = TEMP, _mem=None) -> str:
    """Choose a chord from the recognised expression's available pool.

    Prefer a reasonably smooth change from the current chord while balancing
    long-term use.  Repeating the current chord is avoided whenever the pool
    contains another choice.
    """
    emotion = normalize_label(label)
    pool = list(dict.fromkeys(emotion_pools.get(emotion, [])))
    if not pool:
        pool = list(dict.fromkeys(emotion_pools.get("neutral", [])))
    if not pool:
        return current_chord or CALM_CHORD

    alternatives = [ch for ch in pool if ch != current_chord]
    candidates = alternatives or pool
    # Small voice-leading distances are preferred, while the usage penalty
    # stops a single chord from dominating every block.
    scores = [
        semitone_dist(current_chord, ch) * 0.35 + CHORD_USE_COUNT[ch] * 0.8
        for ch in candidates
    ]
    weights = softmax_scores(scores, max(float(temp), 0.05))
    chosen = random.choices(candidates, weights=weights, k=1)[0]
    CHORD_USE_COUNT[chosen] += 1
    return chosen
...
def choose_block_layers(pool: List[Dict], chord: str, energy: float,
                        mem: RecentMem, label: str = "neutral"):
    """Select layers using the label-conditioned profile for this block."""
    profile = get_music_profile(label)
    selected = []
    selected_groups = []

    for group_name in profile["groups"]:
        group = LOOP_GROUPS[group_name]
        picked = softmax_pick(pool, chord, energy, group, 1, TEMP, mem)
        selected.extend(picked)
        selected_groups.extend([group_name] * len(picked))

    core, orn = [], []
    for item, group_name in zip(selected, selected_groups):
        if group_name in {"top", "percussion"}:
            orn.append(item)
        else:
            core.append(item)

    print(
        f"  🎚 Profile={normalize_label(label)} | "
        f"target_energy={profile['target_energy']:.2f} | "
...
def build_block(chord: str, pool: List[Dict], target_energy: float,
                dur_ms: int, mem: RecentMem, label: str = "neutral") -> AudioSegment:
    ...
    core_sel, orn_sel = choose_block_layers(pool, chord, target_energy, mem, label)
    ...
    full=core.overlay(orn).fade_in(80).fade_out(CROSSFADE_MS)
        # === Subzero cold & dark EQ ===
    full = full.low_pass_filter(11000)   # remove brightness
    full = full.high_pass_filter(60)     # remove mud
    full = effects.normalize(full)
    full = full.apply_gain(-1)
...
gate_in_progress=threading.Event()
...
def decode_yolo(res, names):
    ...
    if boxes is not None and len(boxes)>0:
        ...
        label=names.get(cls,str(cls)) if isinstance(names,dict) else (names[cls] if 0<=cls<len(names) else str(cls))
...
def performer_worker():
    try:
        ...
        desired_emotion = "neutral"
        energy_base = MUSIC_PROFILES["neutral"]["target_energy"]
        ...
        print(
            "🎛  Expression labels control chord pool, loop profile "
            "and target energy.",
            flush=True,
        )
        print("🎹 Chord pools:", EMO_TO_CHORDS, flush=True)

        # 当前情绪（默认 neutral），用于 OSC
        last_emotion = "neutral"

        seg=build_block(current_chord, pool, energy_base, block_ms, mem, current_emotion)
        h_current=play_block(seg); t_start=time.time()
        built_next=None; h_next=None
        next_emotion=None
        gate_armed=False
        gate_advance = (
            PREBUILD_SEC + (CROSSFADE_MS/1000.0) +
            DETECT_WIN_SEC + DECISION_MARGIN_SEC
        )

        # 第一个 block 也可以先发一次 neutral 给 TD
        send_osc_emotion(last_emotion)

        # 发送文本形式的情绪（TD 显示用）
        if HAVE_OSC and osc_client is not None:
            osc_client.send_message("/emotion_label", last_emotion)
        ...
        while not stop_flag.is_set():
            ...
            try:
                ...
                last_emotion = normalize_label(lbl)
                desired_emotion = last_emotion
                desired_chord = pick_emotion_chord(lbl, current_chord, EMO_TO_CHORDS, TEMP, mem)
                energy_base   = get_music_profile(desired_emotion)["target_energy"]
                # If the decision arrived just after pre-building started,
                # discard the stale block and rebuild it with this label.
                if built_next is not None and h_next is None and next_emotion != desired_emotion:
                    built_next = None
                    next_emotion = None
                    print(
                        "🔁 Rebuilding next block with the newly voted "
                        f"label={desired_emotion}",
                        flush=True,
                    )
                print(
                    f"🪄 Expression label={desired_emotion} → "
                    f"next_chord={desired_chord}  "
                    f"target_energy≈{energy_base:.2f}  "
                    f"groups={','.join(get_music_profile(desired_emotion)['groups'])}",
                    flush=True,
                )
            except queue.Empty:
                pass
            ...
            if built_next is None and elapsed >= (block_ms/1000.0 - PREBUILD_SEC - CROSSFADE_MS/1000.0):
                next_emotion = desired_emotion
                built_next=build_block(desired_chord, pool, energy_base, block_ms, mem, next_emotion)
                print(
                    f"🔧 Prepared next: label={next_emotion} "
                    f"chord={desired_chord}",
                    flush=True,
                )
            ...
            if elapsed >= (block_ms/1000.0 + 0.05):
                ...
                current_emotion = next_emotion if built_next and next_emotion else current_emotion
                h_current = h_next if h_next else h_current
                t_start=time.time()
                built_next=None; h_next=None; next_emotion=None
                gate_armed=False
                detect_gate.clear(); gate_in_progress.clear()

                # ⭐ 在新 block 开始时，把 last_emotion 发给 TD
                send_osc_emotion(current_emotion)

                print(
                    f"▶ Now playing: label={current_emotion} "
...
def ui_loop():
    ...
    title="Emotion (Q: quit | N/H/U/S/A/F/D: inject)"
    ...
    while not stop_flag.is_set():
        ...
        if key in (ord('u'),ord('U')): inject="surprised"
